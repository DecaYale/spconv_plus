...
template <typename Index, typename IndexGrid, unsigned NDim>
struct CreateConcatIndicePairFunctor<tv::CPU, Index, IndexGrid, NDim> {
  Index operator()(const tv::CPU &d, tv::TensorView<const Index> indicesIn1,
                   ...
                   //  bool transpose,
                   bool resetGrid) {
    return getIndicePairsConcat<Index, IndexGrid, NDim>(
        indicesIn1, indicesIn2, gridsOut, indicePairs, indiceNum,
        outSpatialShape.data(),
        outInds
        );
  }
...
#define DECLARE_CPU_SPECS_INDEX_NDIM(Index, NDIM)                              \
  ...
  template struct functor::CreateSubMIndicePairFunctor<tv::CPU, Index, int,    \
                                                       NDIM>;                  \
  template struct functor::CreateConcatIndicePairFunctor<tv::CPU, Index, int,  \
                                                         NDIM>;
